const BASE_URL="https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies";

const dropdown=document.querySelectorAll(".dropdown select");
const btn=document.querySelector("button");
const from=document.querySelector(".from select");
const to=document.querySelector(".to select");
const msg=document.querySelector(".msg");
const amount=document.querySelector("input");


window.addEventListener("load", ()=>{
    updateExchangeRate();
});



for(select of dropdown){
 for(con_name in countryList){
       let option=document.createElement("option");
       option.value=con_name;
       option.innerText=con_name;


       if(select.name==="from" && con_name==="USD"){
        option.selected="selected";
       }
       else if(select.name==="to" && con_name==="PKR"){
        option.selected="selected";
       }

       select.append(option);     
}



      select.addEventListener("change", (evt)=>{
        updateFlag(evt.target);
      });
}



const updateFlag=(element)=>{
        let  con_name=element.value;
        let currcode= countryList[con_name];
        let newsrc=`https://flagsapi.com/${currcode}/flat/64.png`;
       
        let img= element.parentElement.querySelector("img");
        img.src=newsrc;
}


const  updateExchangeRate= async()=>{
    let amountVal=parseFloat(amount.value);
    if(isNaN(amountVal) || amountVal<1){
        amountVal=1;
        amount.value="1";
    }

    try{
    let URL= `${BASE_URL}/${from.value.toLowerCase()}.json`;
    let response= await fetch(URL);
    let data= await response.json();

    let rate= data[from.value.toLowerCase()][to.value.toLowerCase()];
    let finalAmount= amountVal * rate;


    msg.innerText=`${amountVal} ${from.value} = ${finalAmount} ${to.value}`;
}
    catch(error){
        msg.innerText="Error fetching exchange rate.";
        console.error(error);
    }

}



btn.addEventListener("click",  (evt)=> {
   evt.preventDefault();
   updateExchangeRate();
})
